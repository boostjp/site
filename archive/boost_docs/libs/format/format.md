...
- [Syntax](#syntax)
	- [printf format-specifications](#printf-format-specifications)
...
C/C++ の世界におけるレガシーな構文は `printf` で使われているものである。そのため `format` は `printf` の書式文字列をそのまま利用でき、同じ結果を生成する。(ほとんどの場合において。詳細は [Incompatibilities with printf](#differences-of-behavior-vs-printf) を見よ) 
...
### <a name="new-format-specifications" href="new-format-specifications">new format-specifications</a>
...
### <a name="differences-of-behavior-vs-printf" href="differences-of-behavior-vs-printf">Differences of behaviour vs printf</a>

...
- MSVC 14.1は未対応となっていた次の可変長テンプレートベースのコンテナをサポートした: `fusion::vector`, `fusion::deque`, `fusion::map`, `fusion::set`, および `fusion::tuple`. ([#12986](https://svn.boost.org/trac/boost/ticket/12986))
...
- 幾つかのドキュメントを修正 ([PR#50](https://github.com/boostorg/phoenix/pull/50))
- C++11以降かつ連想コンテナで `phoenix::erase` を使用した際にイテレータを返すように変更 ([#7423](https://svn.boost.org/trac/boost/ticket/7423))
- `std::set` および `std::multiset` はphoenixが提供する遅延関数で動作するようになった ([PR#52](https://github.com/boostorg/phoenix/pull/52))
- C++17以降では `phoenix::random_shuffle` を無効化した ([PR#53](https://github.com/boostorg/phoenix/pull/53))
- 前方宣言ではなく適切なヘッダをインクルードするようにした ([#11875](https://svn.boost.org/trac/boost/ticket/11875))
...
- clang-clでのコンパイルエラーを修正 [Track #11821](https://svn.boost.org/trac10/ticket/11821)
